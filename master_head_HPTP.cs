...
public class master_head_HPTP: Photon.MonoBehaviour
{
	...
	void Start ()
	{
		HPLenUnit = HP_bar.GetComponent<RectTransform> ().sizeDelta.x / (float)myOb.GetComponent<PlayerController> ().GetHP();	
		canShow = false;
	...
	{	
		...
		if (myOb != null) {
			...
			HPrt.sizeDelta = new Vector2 (HPLenUnit * myOb.GetComponent<PlayerController> ().GetHP(), HPrt.sizeDelta.y);
